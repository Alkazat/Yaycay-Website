...
const ACCENT: Record<string, string> = { sky: s.sky, sun: s.sun, meadow: s.meadow };
 ...
 * (types the destination, reveals the plan) and then auto-cycles the child tabs
 * until the visitor takes control by clicking a tab.
 */
export function HeroSim() {
  ...
  // The build replay + tab autoplay. Skipped entirely under reduced motion.
  useEffect(() => {
    if (reduced) return;
    const t = timers.current;
    const push = (fn: () => void, ms: number) => t.push(setTimeout(fn, ms));

    // Reset, then type the destination character by character.
    setTyped('');
    setRevealed(false);
    setActive(0);
    const q = sim.query;
  ...
  return (
    ...
    >
      <div className={s.frame}>
        ...
        {/* The plan: a tab per child, each with their own day */}
        <div className={`${s.plan} ${revealed ? s.in : ''}`}>
          <div className={s.tabs} role="tablist" aria-label="Each child's day">
            {sim.kids.map((k, i) => (
              <button
                key={k.name}
                type="button"
              ...
              >
                ...
                {k.name} <span className={s.tabAge}>· {k.age}</span>
              </button>
            ))}
          </div>
